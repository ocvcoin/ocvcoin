...
"""Test framework for ocvcoin utils.
...
def main():
    ...
    bctester(os.path.join(env_conf["SRCDIR"], "test", "util", "data"), "ocvcoin-util-test.json", env_conf)
...
def bctest(testDir, testObj, buildenv):
    ...
    if testObj["exec"] == "./ocvcoin-util":
        execprog = os.getenv("OCVCOINUTIL", default=execprog)
    elif testObj["exec"] == "./ocvcoin-tx":
        execprog = os.getenv("OCVCOINTX", default=execprog)
    ...
    if "error_txt" in testObj:
        ...
        # That stderr is empty if no errors are expected. However, ocvcoin-tx
