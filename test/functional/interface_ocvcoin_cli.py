...
# The block reward of coinbaseoutput.nValue (50) OCV/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 OCV/block.
...
class TestOcvcoinCli(OcvcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `ocvcoin-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and ocvcoin-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and ocvcoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (OCV/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (OCV/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 OCV) to the default wallet.
            ...
            # Test ocvcoin-cli -generate with -rpcwallet in multiwallet mode.
