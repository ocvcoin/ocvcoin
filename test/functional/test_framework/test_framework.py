...
TMPDIR_PREFIX = "ocvcoin_func_test_"
...
class OcvcoinTestMetaClass(type):
    """Metaclass for OcvcoinTestFramework.

    Ensures that any attempt to register a subclass of `OcvcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'OcvcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("OcvcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("OcvcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class OcvcoinTestFramework(metaclass=OcvcoinTestMetaClass):
    """Base class for a ocvcoin test script.

    Individual ocvcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave ocvcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop ocvcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use ocvcoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "ocvcoind": ("ocvcoind", "OCVCOIND"),
            "ocvcoin-cli": ("ocvcoincli", "OCVCOINCLI"),
            "ocvcoin-util": ("ocvcoinutil", "OCVCOINUTIL"),
            "ocvcoin-wallet": ("ocvcoinwallet", "OCVCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: ocvcoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("OcvcoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'ocvcoind', self.options.ocvcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'ocvcoin-cli', self.options.ocvcoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a ocvcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple ocvcoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as ocvcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    ocvcoind=self.options.ocvcoind,
                    ocvcoin_cli=self.options.ocvcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in ocvcoin.conf
    ...
    def skip_if_no_ocvcoind_tracepoints(self):
        """Skip the running test if ocvcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("ocvcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_ocvcoind_zmq(self):
        """Skip the running test if ocvcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("ocvcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if ocvcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("ocvcoin-wallet has not been compiled")

    def skip_if_no_ocvcoin_util(self):
        """Skip the running test if ocvcoin-util has not been compiled."""
        if not self.is_ocvcoin_util_compiled():
            raise SkipTest("ocvcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if ocvcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("ocvcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether ocvcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether ocvcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_ocvcoin_util_compiled(self):
        """Checks whether ocvcoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_OCVCOIN_UTIL")
