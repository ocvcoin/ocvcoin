...
def assert_fee_amount(fee, tx_size, feerate_OCV_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_OCV_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s OCV too low! (Should be %s OCV)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_OCV_kvB)
    if fee > high_fee:
...
def get_fee(tx_size, feerate_ocv_kvb):
    """Calculate the fee in OCV given a feerate is OCV/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_ocv_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  OCV
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "ocvcoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> Tuple[dict, pathlib.Path]:
    ...
    if sys.platform == "win32":
        ...
        datadir = temp_dir / "Ocvcoin"
    else:
        env = dict(HOME=str(temp_dir))
        if sys.platform == "darwin":
            datadir = temp_dir / "Library/Application Support/Ocvcoin"
        else:
            datadir = temp_dir / ".ocvcoin"
    return env, datadir
