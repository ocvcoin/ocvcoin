    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            ...
            for node in legacy_only_nodes:
                # RPC loadwallet failure causes ocvcoind to exit in <= 0.17, in addition to the RPC
        ...
        for wallet_name in ["w1", "w2", "w3"]:
            if self.options.descriptors:
                ...
                node_v16.assert_start_raises_init_error([f"-wallet={wallet_name}"], f"Error: Error loading {wallet_name}: Wallet requires newer version of Ocvcoin Core")
        ...
        for node in descriptors_nodes if self.options.descriptors else legacy_nodes:
            ...
            # Make sure this wallet opens with only the migration warning. See https://github.com/ocvcoin/ocvcoin/pull/19054
