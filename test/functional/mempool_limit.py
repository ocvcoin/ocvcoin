...
class MempoolLimitTest(OcvcoinTestFramework):
    ...
    def run_test(self):
        ...
        worst_feerate_ocvvb = Decimal("21000000")
        for txid in current_mempool:
            entry = node.getmempoolentry(txid)
            worst_feerate_ocvvb = min(worst_feerate_ocvvb, entry["fees"]["descendant"] / entry["descendantsize"])
        ...
        child_fee = (worst_feerate_ocvvb) * (target_weight_each // 4) - Decimal("0.00001")
        ...
        assert_greater_than(worst_feerate_ocvvb, (parent_fee + child_fee) / (tx_parent_just_below["tx"].get_vsize() + tx_child_just_above["tx"].get_vsize()))
