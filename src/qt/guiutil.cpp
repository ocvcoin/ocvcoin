...
#include <qt/ocvcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Ocvcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new OcvcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new OcvcoinAddressCheckValidator(parent));
}
...
bool parseOcvcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no ocvcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("ocvcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!OcvcoinUnits::parse(OcvcoinUnits::OCV, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseOcvcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseOcvcoinURI(uriInstance, out);
}

QString formatOcvcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("ocvcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(OcvcoinUnits::format(OcvcoinUnits::OCV, info.amount, false, OcvcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openOcvcoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", OCVCOIN_CONF_FILENAME));
    ...
    /* Open ocvcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ocvcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ocvcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Ocvcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Ocvcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "ocvcoin.desktop";
    return GetAutostartDir() / strprintf("ocvcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a ocvcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Ocvcoin\n";
        else
            optionFile << strprintf("Name=Ocvcoin (%s)\n", chain);
