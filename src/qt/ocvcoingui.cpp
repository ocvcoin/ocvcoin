...
const std::string OcvcoinGUI::DEFAULT_UIPLATFORM =
...
OcvcoinGUI::OcvcoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &OcvcoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &OcvcoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &OcvcoinGUI::showModalOverlay);
...
void OcvcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Ocvcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ocvcoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &OcvcoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &OcvcoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &OcvcoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Ocvcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Ocvcoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Ocvcoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Ocvcoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a ocvcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Ocvcoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &OcvcoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &OcvcoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &OcvcoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &OcvcoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &OcvcoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &OcvcoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &OcvcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &OcvcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &OcvcoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &OcvcoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &OcvcoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &OcvcoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &OcvcoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &OcvcoinGUI::showDebugWindow);
}

void OcvcoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void OcvcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &OcvcoinGUI::setCurrentWalletBySelectorIndex);
...
void OcvcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &OcvcoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &OcvcoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &OcvcoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &OcvcoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void OcvcoinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void OcvcoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &OcvcoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &OcvcoinGUI::removeWallet);
...
WalletController* OcvcoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void OcvcoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &OcvcoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &OcvcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &OcvcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &OcvcoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &OcvcoinGUI::incomingTransaction);
    connect(this, &OcvcoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void OcvcoinGUI::removeWallet(WalletModel* walletModel)
{
...
void OcvcoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void OcvcoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void OcvcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void OcvcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void OcvcoinGUI::createTrayIcon()
{
...
void OcvcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &OcvcoinGUI::toggleHidden);
...
void OcvcoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void OcvcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void OcvcoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void OcvcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void OcvcoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void OcvcoinGUI::openClicked()
{
...
void OcvcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void OcvcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void OcvcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void OcvcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void OcvcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void OcvcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void OcvcoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void OcvcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Ocvcoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void OcvcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void OcvcoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void OcvcoinGUI::updateHeadersSyncProgressLabel()
{
...
void OcvcoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void OcvcoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &OcvcoinGUI::quitRequested);
...
void OcvcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void OcvcoinGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &OcvcoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void OcvcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void OcvcoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &OcvcoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &OcvcoinGUI::show);
                e->ignore();
            }
        }
    }
...
void OcvcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void OcvcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void OcvcoinGUI::incomingTransaction(const QString& date, OcvcoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(OcvcoinUnits::formatWithUnit(unit, amount, true));
...
void OcvcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void OcvcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool OcvcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool OcvcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void OcvcoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void OcvcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void OcvcoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void OcvcoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void OcvcoinGUI::updateWindowTitle()
{
...
void OcvcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void OcvcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void OcvcoinGUI::detectShutdown()
{
...
void OcvcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void OcvcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(OcvcoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = OcvcoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void OcvcoinGUI::subscribeToCoreSignals()
{
...
void OcvcoinGUI::unsubscribeFromCoreSignals()
{
...
bool OcvcoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<OcvcoinUnit> units = OcvcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const OcvcoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, OcvcoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const OcvcoinUnit u : OcvcoinUnits::availableUnits()) {
        menu->addAction(OcvcoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(OcvcoinUnit newUnits)
{
    setText(OcvcoinUnits::longName(newUnits));
}
