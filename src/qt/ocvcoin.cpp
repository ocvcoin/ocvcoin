...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<OcvcoinUnit>("OcvcoinUnit");
#else
    qRegisterMetaType<OcvcoinUnit>("OcvcoinUnit");
#endif
}
...
bool OcvcoinApplication::createOptionsModel(bool resetSettings)
{
...
void OcvcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    ...
    connect(window, &OcvcoinGUI::quitRequested, this, &OcvcoinApplication::requestShutdown);
...
void OcvcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
void OcvcoinApplication::startThread()
{
    ...
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &OcvcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &OcvcoinApplication::handleRunawayException);
    connect(this, &OcvcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &OcvcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}
...
bool OcvcoinApplication::event(QEvent* e)
{
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6-7. Parse ocvcoin.conf, determine network, switch to network specific
