...
/** Call node.updateRwSetting() with Ocvcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Ocvcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/ocvcoin/ocvcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because ocvcoin 22.x will not try to read these.
...
//! Convert enabled/size values to ocvcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from ocvcoin -prune setting.
...
//! Get pruning size value to show in GUI from ocvcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayOcvcoinUnit")) {
        settings.setValue("DisplayOcvcoinUnit", QVariant::fromValue(OcvcoinUnit::OCV));
    }
    QVariant unit = settings.value("DisplayOcvcoinUnit");
    if (unit.canConvert<OcvcoinUnit>()) {
        m_display_ocvcoin_unit = unit.value<OcvcoinUnit>();
    } else {
        m_display_ocvcoin_unit = OcvcoinUnit::OCV;
        settings.setValue("DisplayOcvcoinUnit", QVariant::fromValue(m_display_ocvcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if ocvcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_ocvcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<OcvcoinUnit>() == m_display_ocvcoin_unit) return;
    m_display_ocvcoin_unit = new_unit.value<OcvcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayOcvcoinUnit", QVariant::fromValue(m_display_ocvcoin_unit));
    Q_EMIT displayUnitChanged(m_display_ocvcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/ocvcoin/ocvcoin/pull/8273
    ...
    // (https://github.com/ocvcoin-core/gui/issues/567).
