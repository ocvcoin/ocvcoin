...
{
    return AbsPathForConfigVal(fs::path(args.GetArg("-pid", OCVCOIN_PID_FILENAME)));
}
...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location. (default: %s)", OCVCOIN_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ocvcoin/ocvcoin>";
...
/** Sanity checks
 *  Ensure that Ocvcoin is running in a usable environment with all
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    fs::path config_file_path = GetConfigFile(args.GetArg("-conf", OCVCOIN_CONF_FILENAME));
