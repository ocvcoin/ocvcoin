...
// For details see https://github.com/ocvcoin/ocvcoin/pull/22348.
...
const char * const OCVCOIN_CONF_FILENAME = "ocvcoin.conf";
const char * const OCVCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/ocvcoin/ocvcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //ocvcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", OCVCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Ocvcoin
    // macOS: ~/Library/Application Support/Ocvcoin
    // Unix-like: ~/.ocvcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ocvcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Ocvcoin";
#else
    // Unix-like
    return pathRet / ".ocvcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", OCVCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Ocvcoin Core copyright is not removed by accident
    if (copyright_devs.find("Ocvcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Ocvcoin Core developers";
    }
