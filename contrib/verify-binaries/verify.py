...
"""Script for verifying Ocvcoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from bitcoincore.org and ocvcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/ocvcoin-core/guix.sigs/tree/main/builder-keys
...
HOST2 = "https://ocvcoin.org"
VERSIONPREFIX = "ocvcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Ocvcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"ocvcoin_verify_binaries.{args.version}"
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the ocvcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes ocvcoin.org lags behind bitcoincore.org.)')
    )
