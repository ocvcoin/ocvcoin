...
###   bitcoincore.org and ocvcoin.org and compares them.
...
WORKINGDIR="/tmp/ocvcoin_verify_binaries"
TMPFILE="hashes.tmp"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ocvcoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "ocvcoin.org failed to provide signature file, but bitcoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ocvcoin.org and bitcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Ocvcoin Core binary release signing key installed?"
   fi
