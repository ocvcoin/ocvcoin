...
""" Example logging Ocvcoin Core mempool events using the mempool:added,
...
def main(ocvcoind_path):
    ocvcoind_with_usdts = USDT(path=str(ocvcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    ocvcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    ocvcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    ocvcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    ocvcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[ocvcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/ocvcoind")
        exit(1)
