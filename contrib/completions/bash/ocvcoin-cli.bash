# bash programmable completion for ocvcoin-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $ocvcoin-cli for RPC
_ocvcoin_rpc() {
...
_ocvcoin_cli() {
    ...
    # save and use original argument to invoke ocvcoin-cli for -help, help and RPC
    # as ocvcoin-cli might not be in $PATH
    ocvcoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($ocvcoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_ocvcoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _ocvcoin_cli ocvcoin-cli
