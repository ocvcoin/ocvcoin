...
def parse_config_into_ocv_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Ocvcoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/ocvcoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/ocvcoin_config.h'), "w", encoding="utf8") as ocv_config:
        ocv_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Ocvcoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_ocv_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/ocvcoin_config.h'), os.path.join(SOURCE_DIR, 'config/ocvcoin-config.h'))
