...
Introduction
---------------------
Visual Studio 2022 is minimum required to build Ocvcoin Core.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build Ocvcoin Core from the command-line, it is sufficient to only install the [Visual Studio Build Tools](https://visualstudio.microsoft.com/downloads/) component.
...
Qt
---------------------
To build Ocvcoin Core with the GUI, a static build of Qt is required.
...
```cmd
msbuild build_msvc\ocvcoin.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```
