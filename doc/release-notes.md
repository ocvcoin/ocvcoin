...
using the Linux kernel, macOS 11.0+, and Windows 7 and newer.  Ocvcoin
...
- `ocvcoind` and `ocvcoin-qt` will now raise an error on startup
 if a datadir that is being used contains a ocvcoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a ocvcoin.conf file. The error message is just a diagnostic intended
 ...
 the ocvcoin.conf contained in it. (#27302)
...
- A new `ocvcoinconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `ocvcoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of Ocvcoin Core and import the data into a new wallet.
...
```sh
src/ocvcoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/ocvcoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
