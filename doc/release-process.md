...
* On both the master branch and the new release branch:
  ...
  - update `CLIENT_VERSION_MAJOR`, `PACKAGE_VERSION`, and `PACKAGE_STRING` in [`build_msvc/ocvcoin_config.h`](/build_msvc/ocvcoin_config.h)
...
- Translations on Transifex
    - Create [a new resource](https://www.transifex.com/ocvcoin/ocvcoin/content/) named after the major version with the slug `[ocvcoin.qt-translation-<RRR>x]`, where `RRR` is the major branch number padded with zeros. Use `src/qt/locale/ocvcoin_en.xlf` to create it.
    ...
    - Make an announcement that translators can start translating for the new version. You can use one of the [previous announcements](https://www.transifex.com/ocvcoin/ocvcoin/announcements/) as a template.
...
To tag the version (or release candidate) in git, use the `make-tag.py` script from [ocvcoin-maintainer-tools](https://github.com/ocvcoin-core/ocvcoin-maintainer-tools). From the root of the repository run:
    ...
    ../ocvcoin-maintainer-tools/make-tag.py v(new version, e.g. 0.20.0)
...
Generate the change log. As this is a huge amount of work to do manually, there is the `list-pulls` script to do a pre-sorting step based on github PR metadata. See the [documentation in the README.md](https://github.com/ocvcoin-core/ocvcoin-maintainer-tools/blob/master/README.md#list-pulls).
...
Add other builders keys to your gpg keyring, and/or refresh keys: See `../ocvcoin/contrib/builder-keys/README.md`.
...
- Update other repositories and websites for new version
  ...
  - bitcoincore.org RPC documentation update
      ...
      - Install the new Ocvcoin Core release

      - Run ocvcoind on regtest

      - Clone the [bitcoincore.org repository](https://github.com/ocvcoin-core/bitcoincore.org)

      - Run: `go run generate.go` while being in `contrib/doc-gen` folder, and with ocvcoin-cli in PATH
