# User-space, Statically Defined Tracing (USDT) for Ocvcoin Core

Ocvcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Ocvcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Ocvcoin Core
...
Multiple tools can list the available tracepoints in a `ocvcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Ocvcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/ocvcoind
stap net        outbound_message 0x0000000000107c05 /src/ocvcoind
stap validation block_connected  0x00000000002fb10c /src/ocvcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Ocvcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/ocvcoind | grep NT_STAPSDT -A 4 -B 2
