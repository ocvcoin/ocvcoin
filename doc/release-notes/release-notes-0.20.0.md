...
Ocvcoin Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/ocvcoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `OCVCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Ocvcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Ocvcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Ocvcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Ocvcoin Core. Ocvcoin Core logs debug messages
...
- OpenSSL is no longer used by Ocvcoin Core.  (#17265)

- BIP70 support has been fully removed from Ocvcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/ocvcoin/ocvcoin/issues/16599),
  [PR #16702](https://github.com/ocvcoin/ocvcoin/pull/16702), and the
  `ocvcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Ocvcoin Core starts are now
...
- The "Start Ocvcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Ocvcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Ocvcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/ocvcoin-core/HWI) for
  signing.  Future versions of Ocvcoin Core should support a GUI option
...
- Ocvcoin Core's automatically-generated source code documentation is
...
- The `ocvcoin-cli` utility used with the `-getinfo` parameter now
...
- The `ocvcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `ocvcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_ocvcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBOCVCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_ocvcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBOCVCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for ocvcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate ocvcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest ocvcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_ocvcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (ocvcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone ocvcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Ocvcoin Core files (hebasto)
...
- #17119 Fix broken ocvcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from ocvcoin-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to ocvcoin-wallet (hel-o)
...
- #18341 Replace remaining literal OCV with `CURRENCY_UNIT` (domob1812)
...
- #16115 On ocvcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from ocvcoin-cli -getinfo (malevolent)
