...
Ocvcoin Core version 25.0 is now available from:

  <https://bitcoincore.org/bin/ocvcoin-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/ocvcoin/ocvcoin/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `ocvcoin-cli` tool both take advantage of this, so
  for example:

```sh
ocvcoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
ocvcoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Ocvcoin Core will now return an error at startup. (#25574)
...
before Ocvcoin Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching ocvcoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/ocvcoin/ocvcoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
