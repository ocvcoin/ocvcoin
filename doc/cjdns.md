# CJDNS support in Ocvcoin Core

It is possible to run Ocvcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Ocvcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Ocvcoin network and individual nodes.
...
before it will work with your Ocvcoin Core node. This is described in steps
...
## Run Ocvcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Ocvcoin Core
...
When enabled, this option tells Ocvcoin Core that it is running in an
...
IPv6 local network. This helps Ocvcoin Core perform better address management:
...
CJDNS support was added to Ocvcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `ocvcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/ocvcoin/ocvcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Ocvcoin Core

There are several ways to see your CJDNS address in Ocvcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `ocvcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `ocvcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `ocvcoin-cli help getnodeaddresses` for details.
