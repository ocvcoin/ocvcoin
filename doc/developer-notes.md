...
- [Development guidelines](#development-guidelines)
    - [General Ocvcoin Core](#general-ocvcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Ocvcoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while ocvcoind is running with the `logging`
RPC.  For instance, launching ocvcoind with `-debug` or `-debug=1` will turn on
...
with "play ocvcoins" on a test network.
...
Ocvcoin Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run ocvcoind.

You can then use the `-debug=lock` configuration option at ocvcoind startup or
`ocvcoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `ocvcoin-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/ocvcoin/ocvcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ocvcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ocvcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/ocvcoind -printtoconsole
...
# A coverage report will now be accessible at `./test_ocvcoin.coverage/index.html`,
...
To profile a running ocvcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep ocvcoind` -- sleep 60
```
...
Ocvcoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/ocvcoin/ocvcoin/issues/12691)
...
- [Main thread (`ocvcoind`)](https://doxygen.bitcoincore.org/ocvcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `ocvcoind.cpp`. Responsible for starting up and
...
However, in open source software such as Ocvcoin Core, where everyone uses
...
pay attention to for reviewers of Ocvcoin Core code.

General Ocvcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Ocvcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `OCVCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef OCVCOIN_FOO_BAR_H
#define OCVCOIN_FOO_BAR_H
...
#endif // OCVCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of Ocvcoin Core, in which case
...
a Ocvcoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- src/leveldb
  - Subtree at https://github.com/ocvcoin-core/leveldb-subtree ; maintained by Core contributors.
...
- src/crc32c
  ...
  - Subtree at https://github.com/ocvcoin-core/crc32c-subtree ; maintained by Core contributors.
  - Upstream at https://github.com/google/crc32c ; maintained by Google.

- src/secp256k1
  - Upstream at https://github.com/ocvcoin-core/secp256k1/ ; maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/ocvcoin-core/ctaes ; maintained by Core contributors.
...
file descriptors, it will cause problems with Ocvcoin's `select()` loop, because
...
compatibility between nodes. This happened in Ocvcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Ocvcoin's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/ocvcoin/ocvcoin/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/ocvcoin/ocvcoin/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/ocvcoin/ocvcoin/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ocvcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ocvcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Ocvcoin Core, the wallet was always
...
- Interface method names should be `lowerCamelCase` and standalone function names should be
  ...
  *Rationale*: consistency and friendliness to code generation tools.
  ...
  before in [#14635](https://github.com/ocvcoin/ocvcoin/pull/14635).
