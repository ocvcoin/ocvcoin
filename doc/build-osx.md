...
This guide describes how to build ocvcoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Ocvcoin Core from source.
...
### 4. Clone Ocvcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Ocvcoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `ocvcoind` or  `ocvcoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/ocvcoin/ocvcoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Ocvcoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/ocvcoin/ocvcoin/issues/7714).
...
You can deploy a `.dmg` containing the Ocvcoin Core application using `make deploy`.
...
## Building Ocvcoin Core

### 1. Configuration

There are many ways to configure Ocvcoin Core, here are a few common examples:
...
Run the following in your terminal to compile Ocvcoin Core:
...
## Running Ocvcoin Core

Ocvcoin Core should now be available at `./src/ocvcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/ocvcoin-qt`.

The first time you run `ocvcoind` or `ocvcoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Ocvcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Ocvcoin"

touch "/Users/${USER}/Library/Application Support/Ocvcoin/ocvcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Ocvcoin/ocvcoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Ocvcoin/debug.log
```

## Other commands:

```shell
./src/ocvcoind -daemon      # Starts the ocvcoin daemon.
./src/ocvcoin-cli --help    # Outputs a list of command-line options.
./src/ocvcoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/ocvcoin-qt -server # Starts the ocvcoin-qt server mode, allows ocvcoin-cli control
```
